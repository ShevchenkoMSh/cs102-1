...
def encrypt_vigenere(plaintext: str, keyword: str) -> str:
    ...
    for i in range(len(plaintext)):
        if ord(plaintext[i]) >= firstBigSymbol and ord(plaintext[i]) <= lastBigSymbol:
            ciphertext += chr(
                (ord(plaintext[i]) + (ord(keyword[k]) % firstBigSymbol) - firstBigSymbol) % 26
                + firstBigSymbol
            )
            k = (k + 1) % len(keyword)
        elif ord(plaintext[i]) >= firstSmallSymbol and ord(plaintext[i]) <= lastSmallSymbol:
            ciphertext += chr(
                (ord(plaintext[i]) + (ord(keyword[k]) % firstSmallSymbol) - firstSmallSymbol) % 26
                + firstSmallSymbol
            )
            k = (k + 1) % len(keyword)
        else:
    ...
    for i in range(len(ciphertext)):
        if ord(ciphertext[i]) >= firstBigSymbol and ord(ciphertext[i]) <= lastBigSymbol:
            plaintext += chr(
                (
                    ord(ciphertext[i])
                    - ((ord(keyword[k]) % firstBigSymbol) % firstBigSymbol)
                    - firstBigSymbol
                )
                % 26
                + firstBigSymbol
            ...
            plaintext += chr(
                (ord(ciphertext[i]) - (ord(keyword[k]) % firstSmallSymbol) - firstSmallSymbol) % 26
                + firstSmallSymbol
            )
            k = (k + 1) % len(keyword)
        else:
